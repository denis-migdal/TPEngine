...
export class QFile extends LISS({ html, css: [answer_css, css] }, WithBare, WithContent, (WithRWValue), (WithMeta)) {
    pts = +this.host.getAttribute("pts");
    hasViewer = this.host.getAttribute("viewer") !== "false";
    accepts = this.host.getAttribute('type') ?? "";
    default = this.host.getAttribute("default");
    span_grade = this.content.querySelector('.grade');
    answer = this.content.querySelector('.answer');
    constructor() {
        super();
        // selected
        this.content.querySelector('.upload_btn').addEventListener('click', async () => {
        ...
        this._meta.listen(() => {
            ...
            setGlobalGrade(this.span_grade, meta, this.pts, (grade) => grade * this.pts);
